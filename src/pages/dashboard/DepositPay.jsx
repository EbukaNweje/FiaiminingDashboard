...
const DepositPay = () => {
    ...
    //   wallets?.map((props)=> {walletInfo.wallet === props.walletName ? setH(props?.walletName) : "Loading...";})
      ...
      const handlegetallWalletAddress = async () => {
        await axios.get('https://fiaimining-backend.vercel.app/api/getallWalletAddress')
            .then(response => {
    ...
    useEffect(() => {
      ...
      const [state, setState] = useState({
        value: "bc1q8xh2e0d4dk7pqsvwletn8tvl5vzxg7u0sfud4m",
        copied: false,
      });
    ...
    const url = `https://fiaimining-backend.vercel.app/api/sendpayment/${id}`
      const url2 = `https://fiaimining-backend.vercel.app/api/deposit/${id}
`
      const data = {
